...
import { getSearchUrl, getActionUrl, getUploadUrl, getModelUrl } from '../../services/getUrl';
...
export default function ImportData({ importConfigName, onAlert, refreshData }) {
  ...
  const importData = async () => {
    ...
    const searchImportResponse = await api('POST', getSearchUrl(MODELS.ADVANCED_IMPORT), {
    ...
    if (checkFlashOrError(importActionResponse.data.data)) {
      if (importActionResponse.data.data[0].flash === 'Data imported successfully') {
        ...
        return onAlert('Success', t('LBL_SUCCESS_IMPORT'));
      } else {
        ...
        if (errorMsg) {
          setIsLoading(false);
          setUploadedFile(null);
          return onAlert('Error', t(errorMsg));
        } else {
          commonError();
          return null;
        }
