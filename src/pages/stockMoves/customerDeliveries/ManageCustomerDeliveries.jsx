...
import CustomerDeliveriesForm from './CustomerDeliveriesForm';
...
export default function ManageCustomerDeliveries({ addNew, enableEdit }) {
  ...
  const finishedSaveHandler = status => {
    if (status === 'success') {
      alertHandler('Success', 'LBL_CUSTOMER_DELIVERY_SAVED_SUCCESS');
      setTimeout(() => {
        setIsSave(false);
  ...
  const finishedRealizeHandler = (status, msg) => {
    let message = msg ?? 'LBL_CUSTOMER_DELIVERY_REALIZED_SUCCESS';
  ...
  const finishedInvoiceGenerateHandler = (status, id) => {
    if (status === 'success') {
      ...
      setTimeout(() => {
        navigate(getFeaturePath('CUSTOMERS_INVOICES', 'edit', { id: id, status: 'Draft' }));
      }, 3000);
    } else {
      alertHandler('Error', 'SOMETHING_WENT_WRONG');
    }
  ...
  const finishedReverseMoveHandler = (status, id) => {
    if (status === 'success') {
      alertHandler('Success', 'LBL_REVERSE_CUSTOMER_DELIVERY_SUCCESS');
      setTimeout(() => {
        setReverseStockMove(false);
        navigate(getFeaturePath('CUSTOMER_RETURNS', 'edit', { id: id }));
      }, 3000);
    } else {
      alertHandler('Error', 'SOMETHING_WENT_WRONG');
    }
  ...
  const finishedCancelMoveHandler = status => {
    if (status === 'success') {
      ...
      alertHandler('Success', 'LBL_CANCEL_CUSTOMER_DELIVERY_SUCCESS');
      setTimeout(() => {
        setCancelMove(false);
        navigate(getFeaturePath(subFeature));
      }, 3000);
  ...
  const finishedDeleteHandler = status => {
    if (status === 'success') {
      alertHandler('Success', 'LBL_CUSTOMER_DELIVERY_DELETED');
      setTimeout(() => {
        setIsDelete(false);
        navigate(getFeaturePath(subFeature));
      }, 3000);
  ...
  const getUnrealizedReversions = async () => {
    ...
    if (total && total > 0) {
      return alertHandler('Error', 'LBL_YOU_HAVE_UNREALIZED_CUSTOMER_RETURNS_FROM_THIS_CUSTOMER_DELVIERY');
    } else {
    ...
    <>
          ...
          <div className="row">
            <div className="col-md-12 mb-4">
              <div className="info-tite-page float-start">
                <h4>{addNew ? t('LBL_NEW_CUSTOMER_DELIVERY') : stockMove?.stockMoveSeq ?? ''}</h4>
              </div>
              ...
              <div className="reverse-page float-end">
                ...
                {enableEdit && stockMove && stockMove.statusSelect === 2 && (
                  <PrimaryButton
                    text="LBL_REALIZE_CUSTOMER_DELIVERY"
      ...
      {showInvoiceModal && (
        <GenerateInvoiceModal
          title="LBL_CUSTOMER_DELIVERIES_LINES"
          data={stockMove}
      ...
      {showCancelModal && (
        <CancellationModal
          title="LBL_CANCEL_CUSTOMER_DELIVERY"
