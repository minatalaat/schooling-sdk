...
// need to implement Dropdown with MetaFields here
// import DropDown from '../../../components/ui/inputs/DropDown';
// import useMetaFields from '../../../hooks/metaFields/useMetaFields';

const CustomerDeliveriesForm = ({
  enableEdit,
  data,
  addNew,
  isSave,
  ...
  // const stockMoveTypes = useMetaFields('stock.move.type.select');
    ...
    let payload = {
      ...
      data: {
        ...
        context: {
          ...
          isWithBackorder: true,
          isWithReturnSurplus: false,
          // fromStockLocation: selectedFromStockLocation,
          toStockLocation: null,
          estimatedDate: formik.values.estimatedDate,
          partner: formik.values.partner,
          toAddress: null,
  ...
  const saveRecord = async () => {
    ...
    if (saveDateResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_DELIVERY');

    let saveActionResponse = await api('POST', getActionUrl(), getSaveActionPayload());
    if (saveActionResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_DELIVERY');
    let data = saveActionResponse.data.data?.[0];
    if (data && checkFlashOrError(data)) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_DELIVERY');

    let saveActionOneResponse = await api('POST', getActionUrl(), getSaveActionOnePayload());
    if (saveActionOneResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_DELIVERY');
    ...
    if (saveModelResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_DELIVERY');
  ...
  const realizeRecord = async () => {
    ...
    setActionInProgress(true);
    let saveDateResponse = await api('POST', getModelUrl(MODELS.STOCK_MOVE_ADDITIONAL_DATA), getSaveDateModelPayload());
    if (saveDateResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_DELIVERY');
    ...
    if (realizeActionResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_DELIVERY');
    let data = realizeActionResponse.data.data;
    if (data && checkFlashOrError(data)) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_DELIVERY');

    let verifyResponse = await api('POST', getVerifyUrl(MODELS.STOCK_MOVE), verifyPayload());
    if (verifyResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_DELIVERY');
    data = verifyResponse.data.data;
    if (data && checkFlashOrError(data)) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_DELIVERY');

    let saveModelResponse = await api('POST', getModelUrl(MODELS.STOCK_MOVE), getSaveModelPayload());
    if (saveModelResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_DELIVERY');
    ...
    if (realizeActionOneResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_DELIVERY');
    ...
    if (data && checkFlashOrError(data) && data?.[0]?.flash?.includes('generated (DO')) {
      setParentSaveDone(true);
      setOnSuccessFn('realizePartial');
      return;
    }
    ...
    <>
      <div className="card">
        <div className="row">
          ...
          <div className="col-md-6">
            {/* <DropDown
              options={stockMoveTypes.list}
              formik={formik}
              label="LBL_TYPE"
              accessor="typeSelect"
              mode="view"
              translate={stockMoveTypes.mode === 'enum'}
      ...
      {!addNew && (
        <Tabs
          {...tabsProps}
          tabsList={[
            { accessor: 'cd-lines', label: 'LBL_CUSTOMER_DELIVERIES_LINES' },
            { accessor: 'returns', label: 'LBL_CUSTOMER_RETURNS', isHidden: data.statusSelect < 3 },
          ]}
        >
          <StockMoveLines
            accessor="cd-lines"
            ...
            formik={formik}
            tableTitle="LBL_CUSTOMER_DELIVERIES_LINES"
          ...
          />
          <StockReturns accessor="returns" tableTitle="LBL_CUSTOMER_RETURNS" stockMove={data} />
        </Tabs>
      )}
...
export default CustomerDeliveriesForm;
