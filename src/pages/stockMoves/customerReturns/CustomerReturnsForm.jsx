...
const CustomerReturnsForm = ({
  ...
  const saveRecord = async () => {
    ...
    if (saveDateResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_RETURN');

    let saveActionResponse = await api('POST', getActionUrl(), getSaveActionPayload());
    if (saveActionResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_RETURN');
    let data = saveActionResponse.data.data?.[0];
    if (data && checkFlashOrError(data)) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_RETURN');

    let saveActionOneResponse = await api('POST', getActionUrl(), getSaveActionOnePayload());
    if (saveActionOneResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_RETURN');
    data = saveActionOneResponse.data.data?.[0];
    if (data && checkFlashOrError(data)) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_RETURN');

    let saveModelResponse = await api('POST', getModelUrl(MODELS.STOCK_MOVE), getSaveModelPayload());
    if (saveModelResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_SAVING_CUSTOMER_RETURN');
  ...
  const realizeRecord = async () => {
    ...
    setActionInProgress(true);
    let saveDateResponse = await api('POST', getModelUrl(MODELS.STOCK_MOVE_ADDITIONAL_DATA), getSaveDateModelPayload());
    if (saveDateResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');
    ...
    if (realizeActionResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');
    let responseData = realizeActionResponse.data.data;
    if (responseData && checkFlashOrError(responseData)) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');

    let verifyResponse = await api('POST', getVerifyUrl(MODELS.STOCK_MOVE), verifyPayload());
    if (verifyResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');
    let verifyResponseData = verifyResponse.data.data;
    if (verifyResponseData && checkFlashOrError(verifyResponseData)) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');

    let saveModelResponse = await api('POST', getModelUrl(MODELS.STOCK_MOVE), getSaveModelPayload());
    if (saveModelResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');
    ...
    if (realizeResponseStatus !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');
    ...
    if (realizeActionOneData && checkFlashOrError(realizeActionOneData))
      return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');
    ...
    if (realizeActionOneData?.[0]?.view?.title?.includes('Manage backorder')) {
      ...
      if (realizeResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_REALIZING_CUSTOMER_RETURN');
    }
  ...
  const planRecord = async () => {
    ...
    setActionInProgress(true);
    let saveDateResponse = await api('POST', getModelUrl(MODELS.STOCK_MOVE_ADDITIONAL_DATA), getSaveDateModelPayload());
    if (saveDateResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_PLANNING_CUSTOMER_RETURN');
    ...
    if (planActionResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_PLANNING_CUSTOMER_RETURN');
    let data = planActionResponse.data.data;
    if (data && checkFlashOrError(data)) return alertHandler('Error', 'LBL_ERROR_PLANNING_CUSTOMER_RETURN');

    let verifyResponse = await api('POST', getVerifyUrl(MODELS.STOCK_MOVE), verifyPayload());
    if (verifyResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_PLANNING_CUSTOMER_RETURN');
    data = verifyResponse.data.data;
    if (data && checkFlashOrError(data)) return alertHandler('Error', 'LBL_ERROR_PLANNING_CUSTOMER_RETURN');

    let saveModelResponse = await api('POST', getModelUrl(MODELS.STOCK_MOVE), getSaveModelPayload());
    if (saveModelResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_PLANNING_CUSTOMER_RETURN');
    ...
    if (planActionOneResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_PLANNING_CUSTOMER_RETURN');
    ...
    if (planActionTwoResponse.data.status !== 0) return alertHandler('Error', 'LBL_ERROR_PLANNING_CUSTOMER_RETURN');
  ...
  return (
    <>
      ...
      {!addNew && (
        <Tabs {...tabsProps} tabsList={[{ accessor: 'cr-lines', label: 'LBL_CUSTOMER_RETURNS_LINES' }]}>
          <StockMoveLines
            accessor="cr-lines"
            addNew={addNew}
            ...
            formik={formik}
            tableTitle="LBL_CUSTOMER_RETURNS_LINES"
