...
function InventoryTransactionsReport({ feature, subFeature }) {
  ...
  const generateReport = async () => {
    ...
    let reportPaylod = {
      action: 'action-inventory-transaction-report',
      data: {
        ...
        stockLocationId: formik.values.stockLocation ? formik.values.stockLocation.id : null,
        productId: formik.values.product ? formik.values.product.id : null,
          ...
          <div className="row ">
            <div className="col-md-12 mb-4">
              <div className="info-tite-page float-start">
                <h4>{t('LBL_INVENTORY_TRANSACTIONS_REPORT')}</h4>
              </div>
                  ...
                  {pdfURL !== '' && (
                    <div className="col-md-12">
                      <label className="form-label color-text-red" htmlFor="full-name">
                        {`${t('LBL_REPORT_SHOWS_DATA_BASED_ON_REALIZED_DATE_FROM')} ${moment(formik.values.reportFromDate)
                          .locale('en')
                          .format('YYYY-MM-DD')} ${t('LBL_TO')} ${moment(formik.values.reportToDate).locale('en').format('YYYY-MM-DD')}`}
                      </label>
                    </div>
                  )}
