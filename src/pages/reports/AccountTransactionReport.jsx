...
function AccountTransactionsReport({ feature, subFeature }) {
  ...
  const valSchema = Yup.object().shape({
    ...
    reportFromDate: Yup.date().typeError(t('VALID_DATE_FORMAT')).required(t('REPORT_FROM_DATE_VALIDATION_MESSAGE')),
  ...
  const generateReport = async () => {
    ...
    let reportPaylod = {
      action: 'action-transaction-statement-account-report',
      data: {
    ...
    if (formik.values.account) {
      reportPaylod.data = {
        ...reportPaylod.data,
        accountId: formik.values.account?.id || -1,
      };
    }
          ...
          <div className="row ">
            <div className="col-md-12 mb-4">
              <div className="info-tite-page float-start">
                <h4>{t('LBL_ACCOUNT_TRANSACTIONS_REPORT')}</h4>
              </div>
