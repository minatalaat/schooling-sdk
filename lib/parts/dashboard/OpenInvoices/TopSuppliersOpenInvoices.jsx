...
const TopSuppliersOpenInvoices = () => {
  ...
  const fetchTopSuppliers = () => {
    api(
      ...
      getSearchUrl(MODELS.INVOICE),
      {
        ...
        data: {
          _domain: 'self.statusSelect = 3 AND self.operationTypeSelect=1 AND self.companyInTaxTotalRemaining > 0',
        },
        limit: 5,
        offset: 0,
        translate: true,
        ...
        <div className="card section-card card-no-border">
          <div className="card-header-q">
            <h5 className="float-start mb-0">
              {t('TOP')} {t('OPEN_INVOICES')} <span>{t('SUPPLIERS')}</span>
            </h5>
          </div>
          <div className="card-body-q pt-0">
            {topSuppliers.length > 0 &&
              topSuppliers.map(invoice => (
                <div className="cel-top-2" key={invoice.id}>
                  <h4 className="float-start">
                    ...
                    />
                    {`${invoice?.invoiceId} - ${invoice?.['partner.name']}`}
                  </h4>
                  <p className="float-end up">
                    {formatFloatNumber(invoice.amountRemaining)} {t('LBL_SAR_BRIEF')}
                  </p>
                </div>
              ))}
            {topSuppliers.length === 0 && (
...
export default TopSuppliersOpenInvoices;
