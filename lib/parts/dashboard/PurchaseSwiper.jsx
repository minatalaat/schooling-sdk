...
const PurchaseSwiper = ({ windowSize, isOffersAvailable }) => {
  ...
  let slides = [
    {
      title: 'LBL_INVOICE_PURCHASE_SITUATION_WITH_VAT',
    ...
    {
      title: 'LBL_INVOICE_PURCHASE_PAYMENT_CONDITION_DISTRIBUTION',
  ...
  const invoiceSituationPayload = () => {
    let payload = {
      data: {
        ...
        _domainAction: 'dashlet.invoice.purchase.situation',
      },
      fields: ['dataset'],
    };
    return payload;
  ...
  const getInvoiceSituation = async () => {
    ...
    const response = await api('POST', getModelChartUrl(MODELS.CHART_INVOICE_PURCHASE_SITUATION), invoiceSituationPayload());
  ...
  const getInvoicePCDistribution = async () => {
    ...
    const response = await api(
      'POST',
      getModelChartUrl(MODELS.CHART_INVOICE_PURCHASE_PAYMENTCONDITION_DISTRIBUTION),
          ...
          <>
            <div className="row mb-3">
              <div className="col-md-12 text-center">
                <h2 className="dashboard-title">{t('LBL_PURCHASE')}</h2>
              </div>
            </div>
            <div className="row">
              <Swiper
