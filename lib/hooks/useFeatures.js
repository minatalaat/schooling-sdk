...
export const useFeatures = (featureName, subFeatureName) => {
  ...
  const getFeaturePath = (navigateFeature, mode, params) => {
    if (!navigateFeature || !FEATURES[navigateFeature]) return homePath;
    if (!mode) return baseRoute + FEATURES[navigateFeature].BASE_PATH + getInitialSearchParams(FEATURES[navigateFeature]);

    return baseRoute + FEATURES[navigateFeature].getPathWithParams(mode, params);
  };
