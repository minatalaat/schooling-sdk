...
import CustomerReturnsForm from './CustomerReturnsForm';
...
export default function ManageCustomerReturns({ addNew, enableEdit }) {
  ...
  const finishedSaveHandler = status => {
    if (status === 'success') {
      alertHandler('Success', 'LBL_CUSTOMER_RETURN_SAVED_SUCCESS');
      setTimeout(() => {
        setIsSave(false);
  ...
  const finishedPlanHandler = status => {
    if (status === 'success') {
      alertHandler('Success', 'LBL_CUSTOMER_RETURN_PLANNED_SUCCESS');
      setTimeout(() => {
  ...
  const finishedRealizeHandler = (status, msg) => {
    let message = msg ?? 'LBL_CUSTOMER_RETURN_REALIZED_SUCCESS';
  ...
  const finishedCancelMoveHandler = status => {
    if (status === 'success') {
      ...
      alertHandler('Success', 'LBL_CANCEL_CUSTOMER_RETURN_SUCCESS');
      setTimeout(() => {
        setCancelMove(false);
        navigate(getFeaturePath(subFeature));
      }, 3000);
  ...
  const finishedDeleteHandler = status => {
    if (status === 'success') {
      alertHandler('Success', 'LBL_CUSTOMER_RETURN_DELETED');
      setTimeout(() => {
        setIsDelete(false);
        navigate(getFeaturePath(subFeature));
      }, 3000);
  ...
  return (
    <>
          ...
          <div className="row">
            <div className="col-md-12 mb-4">
              <div className="info-tite-page float-start">
                <h4>{addNew ? t('LBL_NEW_CUSTOMER_RETURN') : stockMove?.stockMoveSeq ? stockMove.stockMoveSeq : ''}</h4>
              </div>
              ...
              <div className="reverse-page float-end">
                ...
                {enableEdit && stockMove && stockMove.statusSelect === 1 && (
                  <PrimaryButton text="LBL_PLAN_CUSTOMER_RETURN" disabled={isButtonDisabled} onClick={() => setPlanStockMove(true)} />
                )}
                {enableEdit && stockMove && stockMove.statusSelect === 2 && (
                  <PrimaryButton text="LBL_REALIZE_CUSTOMER_RETURN" disabled={isButtonDisabled} onClick={() => setRealizeStockMove(true)} />
                )}
              </div>
            </div>
          </div>
      ...
      {showCancelModal && (
        <CancellationModal
          title="LBL_CANCEL_CUSTOMER_RETURN"
