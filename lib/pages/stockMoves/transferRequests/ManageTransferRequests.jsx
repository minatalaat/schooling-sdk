...
import TransferRequestsForm from './TransferRequestsForm';
import ActionsProgessBar from '../../../parts/ActionsProgessBar';
import Calendar from '../../../components/ui/Calendar';

import { useAxiosFunction } from '../../../hooks/useAxios';
import { useFeatures } from '../../../hooks/useFeatures';
import { getFetchUrl, getActionUrl, getSearchUrl } from '../../../services/getUrl';
...
export default function ManageTransferRequests({ addNew, enableEdit }) {
  ...
  const [isCopyToStockTransfer, setCopyToStockTransfer] = useState(false);
  ...
  const alertHandler = (title, message) => {
    ...
    if (title !== 'Success' || !message) {
      ...
      if (isCopyToStockTransfer) setCopyToStockTransfer(false);
    }
  };

  const finishedSaveHandler = status => {
    if (status === 'success') {
      alertHandler('Success', 'LBL_TRANSFER_REQUEST_SAVED_SUCCESS');
      setTimeout(() => {
        setIsSave(false);
  ...
  const finishedCopyHandler = (status, id) => {
    if (status === 'success') {
      alertHandler('Success', 'LBL_TRANSFER_REQUEST_COPIED_SUCCESS');
      setTimeout(() => {
        setCopyToStockTransfer(false);
        navigate(getFeaturePath('STOCK_TRANSFERS', 'edit', { id: id }));
      }, 3000);
    } else {
      alertHandler('Error', 'SOMETHING_WENT_WRONG');
    }
  ...
  let isButtonDisabled = isSave || isDelete || isCopyToStockTransfer;
  return (
    <>
      {showMoreAction && (
        <MoreAction
          ...
          <div className="row">
            <div className="col-md-12 mb-4">
              <div className="info-tite-page float-start">
                <h4>{addNew ? t('LBL_NEW_TRANSFER_REQUEST') : stockMove?.stockMoveSeq ?? ''}</h4>
              </div>
              <div className="reverse-page float-end">
                ...
                {(addNew || enableEdit) && (
                  <PrimaryButton
                    text="LBL_COPY_TO_STOCK_TRANSFER"
                    disabled={isButtonDisabled || stockMoveLineList.length === 0}
                    onClick={() => setCopyToStockTransfer(true)}
                  />
                )}
              </div>
            </div>
                ...
                <>
                  {stockMove && !addNew && (
                    <TransferRequestsForm
                      ...
                      data={stockMove}
                      isSave={isSave}
                      isCopyToStockTransfer={isCopyToStockTransfer}
                      finishedSaveHandler={finishedSaveHandler}
                      finishedCopyHandler={finishedCopyHandler}
                      isDelete={isDelete}
