...
import NoOutgoingPaymentsImg from '../../assets/images/icons/Outgoing payments.svg';

const OutgoingPayments = ({ feature, subFeature }) => {
  ...
  const searchPayload = {
    ...
    sortBy: ['-paymentDate'],
    data: {
      _domain: 'self.invoice.paymentMode.inOutSelect = 2 AND self.paymentMode is not null',
    },
    limit: pageSize,
    offset: offset,
    translate: true,
  ...
  const onPaymentsSearchSuccess = response => {
    ...
    if (status !== 0) {
      setOutgoingPayments([]);
      setDisplayedPayments([]);
      return showErrorMessage('LBL_ERROR_LOADING_OUTGOING_PAYMENTS');
    }
  ...
  return (
    <>
      {isLoading && <Spinner />}
      {((!isLoading && searchValue !== '') || (outgoingPayments && outgoingPayments.length > 0)) && (
        <div className="page-body">
          <div className="container-fluid">
            <div className="row">
              <div className="col-md-12">
                <Calendar />
                <BreadCrumb feature="INVOICES" subFeature="OUTGOING_PAYMENTS" />
              </div>
            </div>
            <div className="row">
              <div className="col-md-12 mb-4">
                <div className="info-tite-page float-start">
                  <h4>{t('LBL_OUTGOING_PAYMENTS')}</h4>
                </div>
              </div>
            </div>
            <div className="row">
      ...
      {!isLoading && outgoingPayments && outgoingPayments.length === 0 && searchValue === '' && (
        <NoData
          imgSrc={NoOutgoingPaymentsImg}
          noDataMessage={t('NO_OUTGOING_PAYMENTS_DATA_MESSAGE')}
          showAdd={canAdd}
          addButtontext={t('LBL_ADD_OUTGOING_PAYMENT')}
          addButtonPath="/home"
        />
      )}
    </>
