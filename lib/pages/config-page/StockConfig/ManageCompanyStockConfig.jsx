...
export default function ManageCompanyStockConfig({ addNew }) {
  ...
  const [stockConfigData, setStockConfigData] = useState(null);
  ...
  const fetchStockConfig = async () => {
    ...
    if (stocktConfigDefaultResponse.data.status !== 0 || !stocktConfigDefaultResponse?.data?.data)
      return alertHandler('Error', 'SOMETHING_WENT_WRONG');
    setStockConfigData(stocktConfigDefaultResponse.data.data[0]);
    setIsLoading(false);
    return stocktConfigDefaultResponse.data.data[0];
  };
        ...
        <div className="container-fluid">
          <div className="row">
            <div className="col-md-12">
              ...
              <BreadCrumb feature={feature} subFeature={subFeature} modeText={t('LBL_COMPANY_STOCK_CONFIGURATION')} />
            </div>
          </div>
          <div className="row">
            <div className="col-md-12 mb-4">
              <div className="info-tite-page float-start">
                <h4>{t('LBL_COMPANY_STOCK_CONFIGURATION')}</h4>
              </div>
            ...
            <div className="col-md-12">
              {!isLoading && (
                <>
                  {addNew && (
                    <StockConfigForm
                      data={stockConfigData}
                      isSave={isSave}
                      finishedSaveHandler={finishedSaveHandler}
                      setActionInProgress={setActionInProgress}
                      alertHandler={alertHandler}
                      fetchStockConfig={fetchStockConfig}
                    />
                  )}
                </>
              )}
