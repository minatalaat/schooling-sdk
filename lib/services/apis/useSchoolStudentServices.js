...
export const useSchoolStudentServices = () => {
  ...
  const deleteStudentFromClass = async (schoolId, classId, body, successHandler) => {
    const res = await api(
      'POST',
      getSchools() + '/' + schoolId + '/classes/' + classId + '/students' + '/delete',
      body,
      successHandler,
      null,
      null,
  ...
  const deleteSupervisor = async (body, successHandler) => {
    await api('POST', getSchools() + '/supervisors' + '/0/delete', body, successHandler, null, null, {
      'X-USER-TYPE': 'ADMIN',
    });
  };
