...
const DropDown = ({
  ...
  const handleViewValue = accessor => {
    let value = accessor;
    value = accessor.split('.')[0];

    if (
      typeof formik?.values[value] === 'object'  ||
      formik?.values[value].hasOwnProperty('name') 
    ) {
      return formik?.values[value].name ? formik?.values[value].name : formik?.values[value].label ? formik?.values[value].label : '';
    }

    return formik?.values[value] ? formik?.values[value] : '';
  };
  ...
  return (
    <>
      ...
      {!disabled && mode !== 'view' ? (
        ...
        <>
          <DisabledInput
            inputValue={isProductPage ? handleViewValue(accessor) : viewValue}
            labelValue={t(label)}
